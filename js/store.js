(function(exports) {
	...
	PMStore.prototype.reset = function() {
		this.store.clear();
	}

	PMStore.prototype.guid = function() {
	...
	PMStore.prototype.getDateindex = function() {
		var inx;
		try {
	...
	PMStore.prototype.setDateindex = function(t) {
		var obj = {
			"ts": this.ts(),
			"d": t
		};
	...
	PMStore.prototype.addDateToIndex = function(date) {
	...
	PMStore.prototype.getTemplate = function() {
		var tmpl;
		try {
	...
	PMStore.prototype.rawGetTemplate = function() {
			var empty = {
				"ts": this.ts(),
				"d": {}
			};
	...
	PMStore.prototype.setTemplate = function(t) {
		var obj = {
			"ts": this.ts(),
			"d": t
		};
	...
	PMStore.prototype.rawSetTemplate = function(t) {
		this.template = t.d;
	...
	PMStore.prototype.rawget = function(k) {
		var obj = JSON.parse(this.store.getItem("data-" + k))
		if (obj === null) return null;
		return obj;
	}

	PMStore.prototype.get = function(k) {
		var obj = JSON.parse(this.store.getItem("data-" + k))
		if (obj === null) return null;
	...
	PMStore.prototype.set = function(k, v) {
		var obj = {
			"ts": this.ts(),
			"d": v
		};
	...
	PMStore.prototype.setDateProject = function(date, project, value) {
		var obj;
	...
	PMStore.prototype.getSyncKey = function() {
	...
	PMStore.prototype.setSyncKey = function(synckey) {
		this.store.setItem("synckey", synckey);
		return synckey;
	}

	PMStore.prototype.synckey = function() {
	...
	PMStore.prototype.resetSyncKey = function() {
		return this.setSyncKey(this.synckey());
	}

	PMStore.prototype.setSettings = function(key, value) {
		var s;
		s = this.getSettings();		
		s[key] = value;
	...
	PMStore.prototype.getSettings = function() {
		var s;
		try {
	...
	PMStore.prototype.getSetting = function(key, def) {
		var s;
		s = this.getSettings();			
		if (!s) return def;
	...
	PMStore.prototype.ts = function() {
		return Math.round(new Date().getTime()/1000.0);
	}

	PMStore.prototype.processSyncResponse = function(response) {
		var datekey;
	...
	PMStore.prototype.getSyncRequest = function() {
		 ...
		 */
	PMStore.prototype.aggregate = function(keys) {
